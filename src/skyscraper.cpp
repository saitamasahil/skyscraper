...
void Skyscraper::doPrescrapeJobs()
{
  ...
  if(config.scraper == "arcadedb" && config.threads != 1) {
    ...
    if(config.user.isEmpty() || config.password.isEmpty()) {
      printf("The IGDB scraping module requires free user credentials to work. Read more about that here: 'https://github.com/Gemba/skyscraper/blob/master/docs/SCRAPINGMODULES.md#igdb'\n");
      exit(1);
    }
	...
	printf("\033[1;33mReceived invalid IGDB server response. This can be caused by server issues or maybe you entered your credentials incorrectly in the Skyscraper configuration. Read more about that here: 'https://github.com/Gemba/skyscraper/blob/master/docs/SCRAPINGMODULES.md#igdb'\033[0m\n");
	exit(1);
      }
    } else {
