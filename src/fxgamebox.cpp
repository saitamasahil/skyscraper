...
QImage FxGamebox::applyEffect(const QImage &src, const Layer &layer,
    ...
    overlayFront =
        overlayFront.scaled(front.width(), front.height(),
                            Qt::IgnoreAspectRatio, Qt::SmoothTransformation);

    painter.begin(&front);
    painter.drawImage(
        0, front.height() * borderFactor / 2,
    ...
    overlaySide =
        overlaySide.scaledToHeight(front.height(), Qt::SmoothTransformation);

    QImage side(overlaySide.width(), overlaySide.height(),
                QImage::Format_ARGB32_Premultiplied);
    ...
    if (layer.scaling == "") {
        // Autoscale
        if ((double)sideImage.height() / sideImage.width() >
            (double)side.height() / side.width()) {
            sideImage = sideImage.scaledToHeight(
                side.height() - side.height() * borderFactor,
                Qt::SmoothTransformation);
        } else {
            sideImage =
                sideImage.scaledToWidth(side.width(), Qt::SmoothTransformation);
        }
    } else {
        ...
        if (layer.scaling == "height") {
            ...
            sideImage = sideImage.scaled(side.width(), side.height(),
    ...
    painter.drawImage(side.width() / 2.0 - sideImage.width() / 2.0,
                      side.height() / 2.0 - sideImage.height() / 2.0,
                      sideImage);
