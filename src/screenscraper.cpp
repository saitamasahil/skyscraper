				     ...
				     QString searchName, QString)
{
  ...
  for(int retries = 0; retries < RETRIESMAX; ++retries) {
	...
	if(data.length() > 64) {
	  ...
	  printf("The erroneous answer was written to '/home/USER/.skyscraper/screenscraper_error.json'. If this file contains game data, please consider filing a bug report at 'https://github.com/Gemba/skyscraper/issues' and attach that file.\n");
	}
	jsonErrorFile.close();
      }
